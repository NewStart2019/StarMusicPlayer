...
/* ── 专辑封面 ── */
.album-wrap { position: relative; width: min(200px,22vw); height: min(200px,22vw); flex-shrink: 0; }
...
.disc-groove:nth-child(1){width:90%;height:90%} .disc-groove:nth-child(2){width:78%;height:78%}
.disc-groove:nth-child(3){width:66%;height:66%} .disc-groove:nth-child(4){width:54%;height:54%}
.disc-groove:nth-child(5){width:42%;height:42%} .disc-groove:nth-child(6){width:30%;height:30%}
.disc-groove:nth-child(7){width:20%;height:20%} .disc-groove:nth-child(8){width:10%;height:10%}
.disc-center {
...
.time-display { display: flex; justify-content: space-between; color: var(--t-text3); font-size: 0.78rem; font-family: 'Orbitron',monospace; margin-bottom: 6px; }
...
.ctrl-btn { background: none; border: none; color: var(--t-text2); cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; border-radius: 50%; padding: 0; }
...
.ctrl-play { width: 60px; height: 60px; background: var(--t-play-bg); color: rgba(0,0,0,0.8); border-radius: 50%; box-shadow: 0 0 26px color-mix(in srgb,var(--t-accent1) 40%,transparent); transition: all 0.2s; }
...
.ctrl-list { width: 36px; height: 36px; }
.ctrl-list svg { width: 18px; height: 18px; }
.ctrl-list.active { color: var(--t-accent1); filter: drop-shadow(0 0 5px var(--t-accent1)); }

/* ── 音量行（独立一行）── */
.volume-row { display: flex; align-items: center; gap: 10px; width: 100%; padding: 0 6px; }
.vol-icon { width: 15px; height: 15px; color: var(--t-text2); flex-shrink: 0; }
.volume-slider { -webkit-appearance: none; flex: 1; height: 3px; border-radius: 999px; background: rgba(255,255,255,0.12); outline: none; cursor: pointer; min-width: 0; }
.volume-slider::-webkit-slider-thumb { -webkit-appearance: none; width: 12px; height: 12px; border-radius: 50%; background: var(--t-play-bg); box-shadow: 0 0 6px var(--t-disc-glow); }
...
.pl-header {
  ...
  padding: 12px 16px 10px; border-bottom: 1px solid var(--t-border); flex-shrink: 0;
}
.pl-label { font-family: 'Orbitron',monospace; font-size: 0.68rem; letter-spacing: 3px; color: var(--t-label-color); flex: 1; }
.pl-count { font-size: 0.75rem; color: var(--t-text3); }
.pl-close-btn { background: none; border: none; cursor: pointer; color: var(--t-text3); padding: 2px; border-radius: 4px; display: flex; transition: color 0.2s; }
.pl-close-btn svg { width: 14px; height: 14px; }
.pl-close-btn:hover { color: var(--t-text); }
.pl-scroll { overflow-y: auto; flex: 1; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.1) transparent; }
.pl-item {
  display: flex; align-items: center; gap: 10px;
  padding: 10px 14px; cursor: pointer; transition: background 0.18s;
}
.pl-item:hover { background: rgba(255,255,255,0.05); }
.pl-cur { background: color-mix(in srgb,var(--t-accent1) 10%,transparent) !important; }
.pl-num { font-size: 0.7rem; color: var(--t-text3); font-family: 'Orbitron',monospace; min-width: 20px; flex-shrink: 0; }
.pl-cur .pl-num { color: var(--t-accent1); }
.pl-name { flex: 1; font-size: 0.83rem; color: var(--t-text2); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; }
.pl-cur .pl-name { color: var(--t-accent1); font-weight: 600; }
.pl-del {
  background: none; border: none; cursor: pointer;
  color: var(--t-text3); opacity: 0; padding: 4px;
  border-radius: 4px; display: flex; flex-shrink: 0; transition: all 0.15s;
}
.pl-del svg { width: 13px; height: 13px; }
.pl-item:hover .pl-del { opacity: 1; }
.pl-del:hover { color: #ff5555; background: rgba(255,85,85,0.12); opacity: 1; }
.pl-empty { padding: 28px; text-align: center; color: var(--t-text3); font-size: 0.85rem; }
/* 弹出动画 */
.pl-anim-enter-active { animation: plPop 0.2s cubic-bezier(0.16,1,0.3,1); }
.pl-anim-leave-active { animation: plPop 0.15s ease reverse; }
@keyframes plPop { from{opacity:0;transform:translateY(10px) scale(0.96)} to{opacity:1;transform:translateY(0) scale(1)} }
...
.lyrics-container { flex: 1; overflow-y: scroll; display: flex; flex-direction: column; align-items: center; gap: 2px; scrollbar-width: none; -ms-overflow-style: none; }
.lyrics-container::-webkit-scrollbar { display: none; }
.lyrics-spacer { height: 38vh; flex-shrink: 0; }
.lyric-line { width: 100%; text-align: center; padding: 9px 12px; font-family: inherit; font-size: 1rem; font-weight: 400; color: var(--t-text3); border-radius: 8px; cursor: pointer; transition: all 0.36s cubic-bezier(0.4,0,0.2,1); line-height: 1.55; }
.lyric-line:hover { color: var(--t-text2); background: rgba(255,255,255,0.04); }
.lyric-line.active { color: var(--t-lyric-active); font-size: 1.15rem; font-weight: 600; text-shadow: 0 0 18px var(--t-lyric-glow); transform: scale(1.04); background: color-mix(in srgb,var(--t-lyric-active) 5%,transparent); }
...
@media (max-width: 880px) {
  .player-left  { width: 56%; padding: 26px 22px; gap: 13px; }
...
@media (max-width: 600px) {
  ...
  .mobile-player  { display: flex; flex-direction: column; width: 100%; height: 100%; overflow: hidden; }
  .mobile-tabs    { display: flex; flex-shrink: 0; border-bottom: 1px solid var(--t-border); margin-top: 54px; }
  .m-tab { flex: 1; padding: 11px; background: none; border: none; border-bottom: 2px solid transparent; color: var(--t-text3); font-family: inherit; font-size: 0.88rem; cursor: pointer; transition: all 0.2s; margin-bottom: -1px; letter-spacing: 1px; }
  .m-tab.active { color: var(--t-accent1); border-bottom-color: var(--t-accent1); font-weight: 600; }
  .m-content { flex: 1; overflow-y: auto; min-height: 0; display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 16px 20px 20px; }
  .m-lyrics-content { gap: 0; padding: 12px 14px; }
  .m-lyrics-scroll  { flex: 1; width: 100%; }
  .m-album { width: min(155px,42vw) !important; height: min(155px,42vw) !important; }
  .m-album .ring-outer { inset: -12px; }
  .m-album .ring-mid   { inset: -4px; }
  .m-content .song-info { width: 100%; }
  .m-title { font-size: clamp(0.9rem,4.5vw,1.25rem) !important; white-space: normal !important; display: -webkit-box !important; -webkit-line-clamp: 2 !important; -webkit-box-orient: vertical !important; text-align: center; }
  ...
  .m-controls .ctrl-prev,.m-controls .ctrl-next { width: 36px; height: 36px; }
  .m-volume-row { width: 100%; padding: 0 6px; }
  ...
  .m-pl-sheet .pl-scroll { max-height: calc(70vh - 54px); }
  .m-pl-anim-enter-active { animation: sheetUp 0.28s cubic-bezier(0.16,1,0.3,1); }
  .m-pl-anim-leave-active { animation: sheetUp 0.2s ease reverse; }
